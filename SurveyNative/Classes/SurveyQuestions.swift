...
open class SurveyQuestions {
    ...
    class open func load(_ jsonFileName : String, surveyTheme: SurveyTheme) -> SurveyQuestions? {
        ...
        if let path = Bundle.main.path(forResource: jsonFileName, ofType: "json"), let data = try? Data(contentsOf: URL(fileURLWithPath: path)) {
            do {
                ...
                Logger.log(dict)
                loadedQuestions = load(dict, surveyTheme: surveyTheme)
            } catch {
    ...
    /**
     Load questions from a JSON dictionary
     */
    class open func load(_ json: [String: Any], surveyTheme: SurveyTheme) -> SurveyQuestions? {
        let loadedQuestions = SurveyQuestions(json["questions"] as! [[String : Any?]], submitData: json["submit"] as! [String : String], surveyTheme: surveyTheme)
        if let autoFocus = json["auto_focus_text"] as? Bool {
